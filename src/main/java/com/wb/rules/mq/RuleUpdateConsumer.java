...
public class RuleUpdateConsumer {
    ...
    public void handleRuleUpdate(Message message) {
        String msgId = getMessageId(message);
        if (msgId == null) {
            log.warn("收到无ID消息，直接拒绝");
            throw new AmqpRejectAndDontRequeueException("收到无ID消息");
        }

        RuleUpdateEvent ruleUpdateEvent = null;
        try {
            // 幂等性检查
            ...
            // 解析消息
            ruleUpdateEvent = parseMessage(message);
            ...
            messageLogService.recordFailure(msgId, e.getMessage(), ruleUpdateEvent);
            ...
            throw new RuleException("规则更新处理失败，需要重试：", e);
        }
    }

    // todo 这里需要考虑数据一致性问题
    public void updateRedis(String msgId){
        try {
    ...
    private void validateRuleUpdateEvent(RuleUpdateEvent event) {
        RuleUpdateProducer.validateRuleUpdateEvent(event);
    }
